...
/*
  Create new table NS from table TRANSACTIONS and related tables - TRANSACTION_LINES, ACCOUNTS, CURRENCIES
  Flag rows where ACCOUNTNUMBER = '548201' as Fee
  Filter only those where ACCOUNT_IDs of ACCOUNTS where ACCOUNTNUMBER is in ('315700', '315710', '315720', '315800', '548201')
  Then set TRANSACTION_ID as PK
*/
DROP TABLE IF EXISTS dbfive.dbo.NS;
GO
...
;
GO
ALTER TABLE dbfive.dbo.NS ADD PRIMARY KEY (TRANSACTION_ID); --set TRANSACTION_ID as PK
GO

-- 2 Load Payment Data
-- 2.1 Create Table
/*
  Prepare empty table for CSV (settlements) data
*/
DROP TABLE IF EXISTS dbfive.dbo.PAYMENT;
...
-- 3.2 Match data
/*
  Fills matching table with NS and PAYMENT (CSV data) data on combined key - ORDER_REF, IS_FEE and TRANSACTION_TYPE
*/
...
-- 4.1 Results - Excess
/*
  Records in NS but do not have corresponding match in matching table
*/
SELECT *
FROM dbfive.dbo.NS
...
-- 4.2 Results - Missing
/*
  Records in PAYMENT(CSV data) but do not have corresponding match in matching table
*/
SELECT *
...
/*
  Get data from NS, inner joined with matching table and PAYMENT data (joined to the matching table)
  Get all accounting relevant columns and add custom column (DIFFERENCE) concatenated of different conditions
  filter only data where DIFFERENCE is not '' in other words, show only rows with some difference (date, currency, account, batch number or price)
*/
SELECT *
FROM
(
